...
export const createGroup = async (request, response) => {
  ...
  try {
    ...
    response.status(201).json({ message: `Group "${name}" created successfully!`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    response.status(500).json({ message: 'Failed to create group. Please try again.' });
  }
};
...
export const getUserGroups = async (request, response) => {
  try {
    ...
    response.json({ message: 'Here are your groups!', groups: sanitizedGroups });
  } catch (error) {
    response.status(500).json({ message: 'Failed to fetch groups. Please try again.' });
  }
};
...
export const getGroup = async (request, response) => {
  ...
  try {
    ...
    response.json({ message: 'Group details retrieved successfully!', group: sanitizeGroup(group) });
  } catch (error) {
    res.status(500).json({ message: 'Failed to fetch group. Please try again.' });
  }
};
...
export const updateGroup = async (request, response) => {
  ...
  try {
    ...
    const populatedGroup = await Group.findById(group._id)
      ...
      .populate('createdBy', 'name email')
      .populate('joinRequests', 'name email');
    response.json({ message: `Group "${group.name}" updated successfully!`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    response.status(500).json({ message: 'Failed to update group. Please try again.' });
  }
};
...
export const deleteGroup = async (request, response) => {
  ...
  try {
    ...
    response.json({ message: `Group "${groupName}" has been disbanded.` });
  } catch (error) {
    response.status(500).json({ message: 'Failed to delete group. Please try again.' });
  }
};
...
export const requestToJoinGroup = async (req, res) => {
  ...
  try {
    ...
    if (group.joinRequests.some(id => id.toString() === userId)) {
      return res.status(400).json({ message: 'You’ve already requested to join this group.' });
    }
    ...
    const populatedGroup = await Group.findById(group._id)
      ...
      .populate('createdBy', 'name email')
      .populate('joinRequests', 'name email');
    res.json({ message: `Your request to join "${group.name}" has been sent!`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    res.status(500).json({ message: 'Failed to send join request. Please try again.' });
  }
};
...
export const approveJoinRequest = async (request, response) => {
  ...
  try {
    ...
    const populatedGroup = await Group.findById(group._id)
      ...
      .populate('createdBy', 'name email')
      .populate('joinRequests', 'name email');
    response.json({ message: `${user.name} has been added to "${group.name}"!`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    response.status(500).json({ message: 'Failed to approve join request. Please try again.' });
  }
};
...
export const rejectJoinRequest = async (request, response) => {
  ...
  try {
    ...
    const populatedGroup = await Group.findById(group._id)
      ...
      .populate('createdBy', 'name email')
      .populate('joinRequests', 'name email');
    response.json({ message: `${user.name}'s join request for "${group.name}" has been rejected.`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    response.status(500).json({ message: 'Failed to reject join request. Please try again.' });
  }
};
...
export const leaveGroup = async (request, response) => {
  ...
  try {
    ...
    const populatedGroup = await Group.findById(group._id)
      ...
      .populate('createdBy', 'name email')
      .populate('joinRequests', 'name email');
    response.json({ message: `You’ve left "${group.name}" successfully.`, group: sanitizeGroup(populatedGroup) });
  } catch (error) {
    response.status(500).json({ message: 'Failed to leave group. Please try again.' });
  }
};